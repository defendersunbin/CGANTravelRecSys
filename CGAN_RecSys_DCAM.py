"""
=============================================================================
   Dual Conditional Attention Mechanism for Personalized Trajectory
   Synthesis — Thesis-Compliant Implementation
   (Chung-Ang University, Master's Thesis, Feb. 2026 / Sunbin Shin)
-----------------------------------------------------------------------------
   This file reconstructs every component of the dissertation into a single
   runnable pipeline.  The journal-version code already contained the
   hybrid embedding (HIN / RNN / NCF), the enhanced-condition-vector
   pipeline, the Anchor-and-Expand algorithm and all evaluation metrics.

   Sections 15 & 16 have been rewritten from scratch to match the
   dissertation exactly, adding the components that were missing from the
   journal-version backup:

       ▸ Dual Conditional Attention Mechanism                (§4.2.1, Fig. 4.1)
         - Generator-side ConditionalAttention layer
         - Critic-side   ConditionalAttention layer  (symbiotic)
       ▸ Noise Injection layer with learnable w_noise        (§4.2.1, Table 3.1)
       ▸ Minibatch Discrimination (100 kernels × 5 dim)      (§4.2.1, Table 5.2)
       ▸ Spectral Normalization on every Critic Dense layer  (§4.2.1)
       ▸ WGAN-GP value function (linear Critic output)       (Eq. 3.2, 4.5)
       ▸ Gradient Penalty   L_GP  (λ_GP = 10.0)              (Eq. 3.2, 4.5)
       ▸ Consistency Regularization  L_cons  (λ_cons = 0.5)  (Eq. 3.3)
       ▸ Auxiliary Regressor network A  +  L_aux (λ_aux = 0.3) (Eq. 3.4, 4.6)

   Hyper-parameters follow Table 5.2 verbatim:
       batch_size_cgan = 128,  epochs_cgan = 2000,
       LR(G) = LR(D)   = 0.0001,  n_critic = 5,
       Minibatch Disc. Kernels = 100,  Kernel Dim = 5,
       Auxiliary Loss Weight λ_aux = 0.3,  Consistency Reg. Weight = 0.5,
       Gradient Penalty λ_GP = 10.0,  Noise Dim = 128, α (MMR) = 0.6 / 0.4

   All other pipeline stages (HIN + Metapath2Vec, Route RNN + PCA,
   NCF, Enhanced Condition Vector with Collaborative Augmentation,
   Anchor-and-Expand decoding with MMR scoring + TSP optimization,
   and every evaluation metric) are inherited unchanged from the
   journal-version backup, because those components already matched the
...
from tensorflow.keras.layers import (
    ...
    Dropout, GaussianNoise, Wrapper, Layer, Lambda, Add
)
...
# -----------------------------------------------
# 15. CGAN Generator / Critic
#     (Dual Conditional Attention + Noise Injection +
#      Spectral Normalization + Minibatch Discrimination +
#      Auxiliary Regressor)  ― faithful to the dissertation.
# -----------------------------------------------

# ==================================================================
# 15.1  Conditional Attention Layer  (논문 4.2.1 & Table 3.1)
#   - Query  : user condition vector c_u  (projected)
#   - Key/Val: intermediate latent features
#   - This is the *Generator-side* half of the Dual Conditional
#     Attention mechanism.  The Critic uses the same class but
#     operates on the real/fake latents (thus "symbiotic pair").
# ==================================================================
class ConditionalAttention(Layer):
    """
    Scaled-dot-product attention that uses the condition vector c_u
    as Query, and the features (x) as Key/Value.  Outputs a
    context-aware re-weighting of x, which is then fused with x
    via a residual connection.

    Args:
        d_attn : internal projection dimension for Q/K/V.
    """
    def __init__(self, d_attn=128, name="cond_attention", **kwargs):
        super().__init__(name=name, **kwargs)
        self.d_attn = d_attn

    def build(self, input_shape):
        # input_shape = [(B, feat_dim), (B, cond_dim)]
        feat_shape, cond_shape = input_shape
        feat_dim = int(feat_shape[-1])
        cond_dim = int(cond_shape[-1])

        # W_Q projects the condition vector (논문 Table 3.1: W^Q, W^K, W^V)
        self.W_Q = self.add_weight(
            shape=(cond_dim, self.d_attn),
            initializer="glorot_uniform", trainable=True, name="W_Q"
        )
        # W_K / W_V project the features
        self.W_K = self.add_weight(
            shape=(feat_dim, self.d_attn),
            initializer="glorot_uniform", trainable=True, name="W_K"
        )
        self.W_V = self.add_weight(
            shape=(feat_dim, self.d_attn),
            initializer="glorot_uniform", trainable=True, name="W_V"
        )
        # Output projection (back to feat_dim so we can residual-add)
        self.W_O = self.add_weight(
            shape=(self.d_attn, feat_dim),
            initializer="glorot_uniform", trainable=True, name="W_O"
        )
        self._feat_dim = feat_dim
        super().build(input_shape)

    def call(self, inputs):
        x, c = inputs                           # x:(B,F), c:(B,C)
        Q = tf.matmul(c, self.W_Q)              # (B, d_attn)
        Kk = tf.matmul(x, self.W_K)             # (B, d_attn)
        V = tf.matmul(x, self.W_V)              # (B, d_attn)

        # Scaled dot-product: we only have a single "token" per sample,
        # so attention degenerates to a per-sample gating signal based
        # on the similarity between the condition and the features.
        # This is the standard formulation used for conditional GANs
        # where Q/K/V are vectors rather than sequences.
        scale = tf.math.sqrt(tf.cast(self.d_attn, tf.float32))
        logits = tf.reduce_sum(Q * Kk, axis=-1, keepdims=True) / scale  # (B,1)
        attn_w = tf.nn.sigmoid(logits)                                  # (B,1) gate in [0,1]

        # Apply attention weight to V, then project back to feat_dim
        context = attn_w * V                                            # (B, d_attn)
        context = tf.matmul(context, self.W_O)                          # (B, F)

        # Residual connection ― lets gradients flow even if the
        # attention gate collapses to 0 at the start of training.
        return x + context
    ...
    def get_config(self):
        ...
        cfg.update({"d_attn": self.d_attn})
        return cfg


# ==================================================================
# 15.2  Noise Injection Layer  (논문 4.2.1, Table 3.1: w_noise)
#   - Injects stochastic Gaussian noise into intermediate G layers
#     with a *learnable* scalar weight w_noise.
# ==================================================================
class NoiseInjection(Layer):
    """Learnable-weighted Gaussian noise injection (G-side)."""
    def __init__(self, name="noise_injection", **kwargs):
        super().__init__(name=name, **kwargs)

    def build(self, input_shape):
        # One learnable scalar per feature dim (so G can decide
        # how much noise each dim needs).
        feat_dim = int(input_shape[-1])
        self.w_noise = self.add_weight(
            shape=(feat_dim,), initializer=tf.initializers.Constant(0.1),
            trainable=True, name="w_noise"
        )
        super().build(input_shape)

    def call(self, x, training=None):
        # Always inject noise (논문에서는 training/inference 구분 없이
        # mode-collapse 방지용으로 noise injection을 수행).
        noise = tf.random.normal(shape=tf.shape(x))
        return x + self.w_noise * noise
...
# ==================================================================
# 15.3  Minibatch Discrimination Layer  (논문 4.2.1 & Table 5.2)
#   - num_kernels = 100, kernel_dim = 5  (Table 5.2 그대로)
#   - Allows the Critic to see cross-sample statistics, directly
#     combating mode collapse (Salimans et al., 2016).
# ==================================================================
class MinibatchDiscrimination(Layer):
    def __init__(self, num_kernels=100, kernel_dim=5,
                 name="minibatch_disc", **kwargs):
        super().__init__(name=name, **kwargs)
    ...
    def build(self, input_shape):
        ...
        self.T = self.add_weight(
            shape=(in_dim, self.num_kernels * self.kernel_dim),
            initializer="glorot_uniform", trainable=True, name="T"
        )
        super().build(input_shape)
...
# ==================================================================
# 15.4  Generator  (논문 4.2.1 & Fig. 4.1)
#   Deep MLP with:
#     - projection of z and c_u
#     - *Conditional Attention Layer*  (G-side of Dual Attention)
#     - BatchNorm + LeakyReLU
#     - *Noise Injection Layer*  (mode-collapse mitigation)
#     - final tanh output
# ==================================================================
def build_generator_DualAttn(noise_d, cond_d, out_d):
    z_input = Input(shape=(noise_d,), name="gen_noise_input")
    c_input = Input(shape=(cond_d,),  name="gen_condition_input")

    # 1) Project z and c_u to a common space.
    ...
    # 2) Fuse noise + condition, then apply the G-side Conditional
    #    Attention Layer that re-weights latent features according
    #    to c_u (논문 4.2.1 "The generator's attention module
    #    selectively re-weights the most salient features of the
    #    user's profile ...").
    x = Concatenate()([g_z, g_c])            # (B, 512)
    x = Dense(512)(x)
    x = BatchNormalization()(x)
    x = LeakyReLU(alpha=0.2)(x)

    x = ConditionalAttention(d_attn=128,
                             name="gen_cond_attention")([x, c_input])

    # 3) Deep MLP backbone with Noise Injection (diversity ↑).
    x = Dense(512)(x)
    x = BatchNormalization()(x)
    x = LeakyReLU(alpha=0.2)(x)
    x = NoiseInjection(name="gen_noise_inject_1")(x)

    x = Dense(512)(x)
    x = BatchNormalization()(x)
    x = LeakyReLU(alpha=0.2)(x)
    x = NoiseInjection(name="gen_noise_inject_2")(x)

    # 4) Output projection  →  tanh (논문: "followed by a final
    #    tanh output layer"; matches MinMaxScaler range [-1, 1]).
    out_layer = Dense(out_d, activation='tanh', name="gen_output")(x)
    return Model([z_input, c_input], out_layer, name="Generator_DualAttn")


# ==================================================================
# 15.5  Critic  (논문 4.2.1 & Fig. 4.1)
#   MLP with:
#     - Gaussian noise on the input latent (regularization)
#     - *Conditional Attention Layer*  (C-side of Dual Attention)
#     - Spectral Normalization on every Dense layer (Lipschitz)
#     - *Minibatch Discrimination* (diversity / mode-collapse)
#     - final *linear* scalar output  (Wasserstein, 非 sigmoid)
# ==================================================================
def build_critic_DualAttn(lat_d, cond_d):
    latent_input = Input((lat_d,),  name="disc_latent_input")
    c_input      = Input((cond_d,), name="disc_condition_input")

    # Input-side Gaussian noise (already in the journal version —
    # also mentioned in the thesis: "input route latent (with
    # Gaussian Noise) and condition vector").
    noisy_latent = GaussianNoise(0.05)(latent_input)

    # 1) Conditional Attention (C-side):  the critic's attention
    #    "adaptively concentrates on the most informative
    #    discrepancies between real and generated distributions"
    #    (논문 4.2.1).  We implement this symmetrically to the G-side:
    #    c_u is Query, the latent features are Key/Value.
    x = ConditionalAttention(d_attn=128,
                             name="disc_cond_attention")([noisy_latent, c_input])

    # 2) Concatenate with c_u so the critic is explicitly conditioned.
    x = Concatenate()([x, c_input])

    # 3) Spectral-Normalized dense backbone  (Lipschitz ⇒ WGAN-GP).
    ...
    # 4) Minibatch Discrimination ― 100 kernels × 5 dim  (Table 5.2).
    x = MinibatchDiscrimination(num_kernels=100, kernel_dim=5,
                                name="disc_mbd")(x)

    x = SpectralNormalization(Dense(128))(x)
    x = LeakyReLU(alpha=0.2)(x)
...
# ==================================================================
# 15.6  Auxiliary Regressor  A  (논문 Eq. 3.4 & 4.6)
#   "An auxiliary network A is trained to reconstruct key attributes
#    of the real latent vector (z_{r,target}) from the generated
#    fake vector  ẑ_r.  This forces the generator to embed more
#    meaningful and condition-relevant information into its output."
#   Implementation: MLP that maps fake latent → latent target dims.
#   We treat ALL latent dims as "key attributes" (the most general
#   realisation of the loss described in Eq. 3.4 / 4.6).
# ==================================================================
def build_auxiliary_regressor(lat_d, target_d=None):
    target_d = target_d if target_d is not None else lat_d
    lat_in = Input(shape=(lat_d,), name="aux_fake_latent_input")
    h = Dense(256)(lat_in)
    h = LeakyReLU(alpha=0.2)(h)
    h = Dense(128)(h)
    h = LeakyReLU(alpha=0.2)(h)
    pred = Dense(target_d, activation='linear', name="aux_output")(h)
    return Model(lat_in, pred, name="AuxiliaryRegressor_A")


# ==================================================================
# 15.7  Instantiate G / C / A
# ==================================================================
if latent_dim_cgan <= 0 or cond_dim <= 0:
    print("CRITICAL ERROR: CGAN dimensions are invalid.")
    G_lat, D_lat, A_reg = None, None, None
    # Keep these names for backward-compatibility with old eval code:
    E_cond, CGAN_model = None, None
else:
    # Thesis hyper-params (Table 5.2):
    #   LR(G) = LR(D) = 0.0001   ;   Adam β1 = 0.5
    g_optimizer = Adam(learning_rate=0.0001, beta_1=0.5, beta_2=0.9)
    d_optimizer = Adam(learning_rate=0.0001, beta_1=0.5, beta_2=0.9)

    G_lat = build_generator_DualAttn(noise_dim, cond_dim, latent_dim_cgan)
    D_lat = build_critic_DualAttn(latent_dim_cgan, cond_dim)   # "Critic" in WGAN terminology
    A_reg = build_auxiliary_regressor(latent_dim_cgan, latent_dim_cgan)

    G_lat.summary()
    D_lat.summary()
    A_reg.summary()

    # Backward-compat aliases so downstream eval code that references
    # `E_cond` or `CGAN_model` does not crash.  They are *unused* in
    # the thesis-compliant training loop below.
    E_cond = None
    CGAN_model = None

# -----------------------------------------------
# 16. CGAN training loop
#     (WGAN-GP  +  Gradient Penalty  +  Consistency Reg.  +
#      Auxiliary Regressor)  ― 논문 Eq. 3.2 / 3.3 / 3.4 / 4.5 / 4.6
# -----------------------------------------------

# === 논문 Table 5.2: 하이퍼파라미터 설정 ===
batch_size_cgan = 128            # Batch Size (CGAN)
epochs_cgan      = 2000          # Epochs (CGAN)
n_critic         = 5             # Standard WGAN-GP: 5 critic updates per G update
lambda_gp        = 10.0          # Gradient Penalty weight λ_GP
...
cons_losses_hist = []
aux_losses_hist = []
w_dist_hist = []      # Estimated Wasserstein distance E[C(real)] - E[C(fake)]
# We also keep this name for legacy plots; it stores |W-dist estimate|
# as a stand-in for the old discriminator-accuracy plot.
c_losses_hist = []
d_accs_hist   = []


# ==================================================================
# 16.1  Gradient Penalty  (논문 Eq. 3.2 / 4.5)
#   LGP = E[ (||∇_{ẑ} C(ẑ, c)||₂ - 1)² ]
#   where  ẑ = ε·z_real + (1-ε)·z_fake,  ε ~ U[0,1].
# ==================================================================
@tf.function
def gradient_penalty(critic, real_lat, fake_lat, cond):
    batch_size = tf.shape(real_lat)[0]
    epsilon = tf.random.uniform((batch_size, 1), 0.0, 1.0)
    interpolated = epsilon * real_lat + (1.0 - epsilon) * fake_lat
    with tf.GradientTape() as gp_tape:
        gp_tape.watch(interpolated)
        pred = critic([interpolated, cond], training=True)
    grads = gp_tape.gradient(pred, interpolated)                  # (B, lat_d)
    grad_norm = tf.sqrt(tf.reduce_sum(tf.square(grads), axis=1) + 1e-12)
    return tf.reduce_mean(tf.square(grad_norm - 1.0))


# ==================================================================
# 16.2  Critic training step  (Eq. 4.5):
#   L_C = E[C(ẑ|c)] - E[C(z|c)]   +  λ_GP · L_GP   +  λ_cons · L_cons
#
#   L_cons (Eq. 3.3):
#   L_cons = E_{z, c, z'} [ || C(z, c) - C(z', c) ||² ]
#   z' = z + N(0, σ²I)      (small perturbation of the real latent)
# ==================================================================
@tf.function
def train_critic_step(real_lat, cond, z_noise):
    # Perturb real latents for consistency regularization.
    z_prime = real_lat + tf.random.normal(tf.shape(real_lat), mean=0.0,
                                          stddev=perturb_std)
    with tf.GradientTape() as c_tape:
        fake_lat = G_lat([z_noise, cond], training=False)

        c_real       = D_lat([real_lat, cond],  training=True)
        c_fake       = D_lat([fake_lat, cond],  training=True)
        c_real_pert  = D_lat([z_prime,  cond],  training=True)

        # 1) Wasserstein distance estimate  (논문 Eq. 3.2 / 4.5)
        wasserstein_est = tf.reduce_mean(c_real) - tf.reduce_mean(c_fake)
        w_loss = tf.reduce_mean(c_fake) - tf.reduce_mean(c_real)   # = -wasserstein_est

        # 2) Gradient Penalty
        gp = gradient_penalty(D_lat, real_lat, fake_lat, cond)

        # 3) Consistency Regularization  (논문 Eq. 3.3)
        cons = tf.reduce_mean(tf.square(c_real - c_real_pert))

        total_c_loss = w_loss + lambda_gp * gp + lambda_cons * cons

    c_grads = c_tape.gradient(total_c_loss, D_lat.trainable_variables)
    d_optimizer.apply_gradients(zip(c_grads, D_lat.trainable_variables))
    return total_c_loss, w_loss, gp, cons, wasserstein_est


# ==================================================================
# 16.3  Generator + Auxiliary Regressor training step  (Eq. 4.6):
#   L_G = - E[C(ẑ|c)]  +  λ_aux · L_aux
#
#   L_aux (Eq. 3.4):
#   L_aux = E[ || A(G(z, c)) - z_{r,target} ||² ]
#
#   where z_{r,target} are key attributes of the *real* latent.
#   (Here we take all dims of the real latent as the target, which
#    is the most general realization of the thesis formulation.)
#
#   A_reg is trained jointly with G so the two networks co-adapt
#   (논문: "An auxiliary network A is trained to reconstruct ...").
# ==================================================================
@tf.function
def train_generator_step(cond, real_lat_target, z_noise):
    with tf.GradientTape() as g_tape:
        fake_lat  = G_lat([z_noise, cond], training=True)
        c_fake    = D_lat([fake_lat, cond], training=False)

        # Adversarial loss  (Eq. 4.6 first term)
        adv_loss  = -tf.reduce_mean(c_fake)

        # Auxiliary regression loss  (Eq. 3.4 / 4.6 second term)
        pred_z    = A_reg(fake_lat, training=True)
        aux_loss  = tf.reduce_mean(tf.square(pred_z - real_lat_target))

        total_g_loss = adv_loss + lambda_aux * aux_loss

    # Update both G and A  (A is trained jointly with G per 논문).
    g_vars = G_lat.trainable_variables + A_reg.trainable_variables
    g_grads = g_tape.gradient(total_g_loss, g_vars)
    g_optimizer.apply_gradients(zip(g_grads, g_vars))
    return total_g_loss, adv_loss, aux_loss


# ==================================================================
# 16.4  Main training loop
# ==================================================================
if G_lat and D_lat and A_reg \
   and X_lat_train_cgan_input.shape[0] > 0 \
   and X_cond_train_scaled.shape[0] > 0:

    print("\n" + "=" * 72)
    print(" Starting CGAN Training — Dual Conditional Attention +")
    print(" WGAN-GP  +  Consistency Reg.  +  Auxiliary Regressor")
    print(f"  epochs={epochs_cgan}, batch={batch_size_cgan}, n_critic={n_critic},")
    print(f"  λ_GP={lambda_gp}, λ_cons={lambda_cons}, λ_aux={lambda_aux}")
    print("=" * 72)

    N_train = X_lat_train_cgan_input.shape[0]

    for epoch_num in range(1, epochs_cgan + 1):

        # --------------------------------------------------------------
        # (A) Train the Critic n_critic times per Generator update.
        #     This is the standard WGAN-GP schedule.
        # --------------------------------------------------------------
        last_c_total, last_w, last_gp, last_cons, last_west = 0, 0, 0, 0, 0
        for _ in range(n_critic):
            idx_batch = np.random.randint(0, N_train, batch_size_cgan)
            real_lat_batch  = tf.convert_to_tensor(
                X_lat_train_cgan_input[idx_batch], dtype=tf.float32)
            real_cond_batch = tf.convert_to_tensor(
                X_cond_train_scaled[idx_batch], dtype=tf.float32)
            z_noise_c = tf.random.normal((batch_size_cgan, noise_dim),
                                         dtype=tf.float32)
            (last_c_total, last_w, last_gp, last_cons, last_west) = \
                train_critic_step(real_lat_batch, real_cond_batch, z_noise_c)

        # --------------------------------------------------------------
        # (B) Train the Generator + Auxiliary Regressor
        # --------------------------------------------------------------
        idx_g_batch = np.random.randint(0, N_train, batch_size_cgan)
        cond_batch_g     = tf.convert_to_tensor(
            X_cond_train_scaled[idx_g_batch], dtype=tf.float32)
        real_lat_g_batch = tf.convert_to_tensor(
            X_lat_train_cgan_input[idx_g_batch], dtype=tf.float32)
        z_noise_g = tf.random.normal((batch_size_cgan, noise_dim),
                                     dtype=tf.float32)
        g_total, adv_loss_val, aux_loss_val = \
            train_generator_step(cond_batch_g, real_lat_g_batch, z_noise_g)

        # --------------------------------------------------------------
        # (C) Record & print
        # --------------------------------------------------------------
        d_losses_hist.append(float(last_c_total))
        g_losses_hist.append(float(adv_loss_val))
        gp_losses_hist.append(float(last_gp))
        cons_losses_hist.append(float(last_cons))
        aux_losses_hist.append(float(aux_loss_val))
        w_dist_hist.append(float(last_west))

        # Back-compat histories (old plots reference these):
        c_losses_hist.append(float(aux_loss_val))
        d_accs_hist.append(float(last_west))     # proxy for "convergence"

        if epoch_num % 100 == 0 or epoch_num == 1:
            print(f"Epoch {epoch_num:>5d}/{epochs_cgan} | "
                  f"C_total: {float(last_c_total):+.4f} | "
                  f"W-dist: {float(last_west):+.4f} | "
                  f"GP: {float(last_gp):.4f} | "
                  f"L_cons: {float(last_cons):.4f} | "
                  f"G_adv: {float(adv_loss_val):+.4f} | "
                  f"L_aux: {float(aux_loss_val):.4f}")

    # ----------------------------------------------------------------
    # Post-training: generate fake latents for visualization / eval
    # ----------------------------------------------------------------
    noise_vis = np.random.normal(0, 1, (N_train, noise_dim))
    fake_latent_scaled = G_lat.predict([noise_vis, X_cond_train_scaled], verbose=0)
    fake_latent = fake_latent_scaled

    # 1. Loss curves  (논문 Fig. 5.12: CGAN training stability)
    plt.figure(figsize=(10, 5))
    plt.plot(d_losses_hist,  label='Critic total loss',       alpha=0.8)
    plt.plot(g_losses_hist,  label='Generator adv. loss',     alpha=0.8)
    plt.plot(gp_losses_hist, label='Gradient Penalty (L_GP)', alpha=0.6)
    plt.plot(cons_losses_hist, label='Consistency Reg. (L_cons)', alpha=0.6)
    plt.plot(aux_losses_hist,  label='Auxiliary Reg. (L_aux)',    alpha=0.6)
    plt.title('CGAN Training Stability — Dual Conditional Attention WGAN-GP')
    ...
    plt.plot(w_dist_hist, label='Est. Wasserstein distance  E[C(real)] − E[C(fake)]',
             color='darkgreen')
    plt.axhline(0, color='grey', linestyle='--', linewidth=0.8)
    plt.title('Wasserstein Distance Estimate over Training')
    plt.xlabel('Iteration')
    plt.ylabel('W-distance estimate')
    plt.legend()
    plt.tight_layout()
